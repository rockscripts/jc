...
class ResPartner(models.Model):
    ...
    @api.onchange('country_id', 'identification_document', 'check_digit', 'l10n_latam_identification_type_id')
    def _onchange_vat(self):
        if self.country_id and self.identification_document:
            if self.country_id.code:
                if self.check_digit and self.l10n_latam_identification_type_id.name == 'NIT':
                    self.vat = self.identification_document + "-" + self.check_digit
                elif self.l10n_latam_identification_type_id.name  == 'Nit de otro país':
